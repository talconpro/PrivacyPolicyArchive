...
onMounted(async () => {
  loading.value = true
  try {
    const res = await api.get(`/apps/${route.params.slug}`)
    app.value = res.data
  } catch (error) {
...
<template>
  ...
  <div v-else class="panel p-6 text-sm text-slate">
    <span v-if="loading">加载中...</span>
    <span v-else>{{ message || '未找到应用信息' }}</span>
  </div>
</template>
